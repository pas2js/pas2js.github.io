...
rtl.module("Classes",["System","RTLConsts","Types","JS"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  $mod.$init = function () {
...
rtl.module("uSanta",["System","Classes","Types","JS","Web"],function () {
  "use strict";
  var $mod = this;
  var $impl = $mod.$impl;
  this.Frame = function (timeStamp) {
    $impl.Update();
    window.requestAnimationFrame($mod.Frame);
  };
  this.MyKeyDown = function (e) {
    if (e.keyCode === 27) {
      ...
      $mod.speed = 20;
      $mod.playMp3("assets\/boing.mp3",false);
    };
  };
  this.GetPicture = function (fileName) {
    var Result = null;
    var img = null;
    img = new Image();
    img.src = fileName;
  ...
  this.playMp3 = function (mp3, loop) {
    var Result = null;
    var snd = null;
    snd = new Audio();
    snd.src = mp3;
  ...
  $impl.Update = function () {
    ...
    var a3 = 0;
    var a4 = 0;
    $mod.ctx.clearRect(0,0,$mod.c.width,$mod.c.height);
    if (($mod.jump > 0) || ($mod.speed > 0)) {
      $mod.jump = $mod.jump + $mod.speed;
      $mod.speed = $mod.speed - 1;
      if ($mod.jump < 0) $mod.jump = 0;
    };
    a1 = $mod.a % 640;
    a2 = Math.floor($mod.a / 2) % 640;
    a3 = Math.floor($mod.a / 3) % 640;
    a4 = Math.floor($mod.a / 4) % 640;
    $mod.ctx.drawImage($mod.bg,-a2,0);
    $mod.ctx.drawImage($mod.bg,-a2 + 640,0);
    $mod.ctx.drawImage($mod.clouds,-a4,50);
    $mod.ctx.drawImage($mod.clouds,-a4 + 640,50);
    if ($mod.jump > 0) {
      $mod.ctx.drawImage($mod.img2,100,130 - $mod.jump)}
     else $mod.ctx.drawImage($mod.img2,100,130 + (5 * Math.sin($mod.a / 10)));
    $mod.ctx.drawImage($mod.fence,-a1,400);
    $mod.ctx.drawImage($mod.fence,-a1 + 640,400);
    $mod.a = $mod.a + 2;
    for (var $l1 = 0, $end2 = rtl.length($mod.arr) - 1; $l1 <= $end2; $l1++) {
      i = $l1;
      $mod.arr[i].y = $mod.arr[i].y + pas.System.Random(5);
      $mod.arr[i].x = ($mod.arr[i].x + pas.System.Random(5)) - 2;
      $mod.ctx.drawImage($mod.flake,$mod.arr[i].x,$mod.arr[i].y);
      if ($mod.arr[i].y > 480) {
        $mod.arr[i].y = -10;
        $mod.arr[i].x = pas.System.Random(640);
      };
    };
    if (($mod.a % 2000) > 200) {
      a1 = $mod.a % 2000;
      $mod.ctx.font = "60px Princess Sofia";
      if (a1 < 640) {
        $mod.ctx.fillText("Powered by pas2js 1.5.1",640 - (a1 - 50),50)}
       else if (a1 < 1000) $mod.ctx.fillText("Powered by pas2js 1.5.1",50,50);
    };
  };
});
rtl.module("program",["System","JS","Classes","Web","uSanta"],function () {
  "use strict";
  var $mod = this;
  this.startBtn = function (Event) {
    var Result = false;
    var i = 0;
    var v = 0;
    var parentEl = null;
    ...
    pas.uSanta.c = document.createElement("CANVAS");
    pas.uSanta.c.width = 640;
    pas.uSanta.c.height = 480;
    pas.uSanta.ctx = pas.uSanta.c.getContext("2d");
    parentEl.appendChild(pas.uSanta.c);
    pas.uSanta.ctx.beginPath();
    pas.uSanta.ctx.arc(95,50,40,0,2 * 3.1416);
    pas.uSanta.ctx.stroke();
    window.requestAnimationFrame(pas.uSanta.Frame);
    window.addEventListener("keydown",pas.uSanta.MyKeyDown);
    pas.uSanta.a = 0;
    v = 2;
    pas.uSanta.speed = 0;
    pas.uSanta.jump = 0;
    pas.uSanta.arr = rtl.arraySetLength(pas.uSanta.arr,pas.Types.TPoint,20);
    for (var $l1 = 0, $end2 = rtl.length(pas.uSanta.arr) - 1; $l1 <= $end2; $l1++) {
      i = $l1;
      pas.uSanta.arr[i].y = pas.System.Random(300);
      pas.uSanta.arr[i].x = pas.System.Random(640);
    };
    pas.uSanta.img = pas.uSanta.GetPicture("assets\/sled.png");
    pas.uSanta.img2 = pas.uSanta.GetPicture("assets\/sled2.png");
    pas.uSanta.flake = pas.uSanta.GetPicture("assets\/snowflake.png");
    pas.uSanta.bg = pas.uSanta.GetPicture("assets\/bg.png");
    pas.uSanta.clouds = pas.uSanta.GetPicture("assets\/clouds.png");
    pas.uSanta.fence = pas.uSanta.GetPicture("assets\/fence.png");
    pas.uSanta.playMp3("assets\/jingle_bells.mp3",true);
    return Result;
  };
  this.el = null;
  $mod.$main = function () {
    $mod.el = document.querySelector("#el2");
    $mod.el.addEventListener("click",$mod.startBtn);
  };
});
//# sourceMappingURL=projectSantaClaus.js.map
